 ...
 */
public class AudioFileSurfaceView extends SurfaceView implements SurfaceHolder.Callback{

    private final String TAG="AudioFileSurfaceView ";
...
//				LogUtils.v("tempCount="+tempCount+" size="+inBuf.size());
        ...
        public  void run() {
            while(isRunning){
                synchronized (lock){
                    if (!holder.getSurface().isValid() || AudioFileSurfaceView.this.isRunning){
                        continue;
                    }
                    if (tempCount!=0){
                        if (!isFullScreen){
                            if(tempFlag){
                                ...
                                for (int i=0;i<3;i++){
                                    tempFlag=false;
                                    AudioFileSurfaceView.this.isRunning=true;
                                    drawWaveForm(inBuf);
                                    AudioFileSurfaceView.this.isRunning=false;
                                }
                            }
                        }
                    }else{
...
//		LogUtils.v("hasReadAudioDataWidth "+hasReadAudioDataWidth);
                   ...
                   /* if (i==inBuf.size()){
                        ...
                        AudioFileSurfaceView.this.reDraws();
                    }*/
                    try {
                        lock.wait();
    ...
    private AudioFileSurfaceView.DrawHasReadAudioData mDrawHasReadAudioData;
    public void startHasReadAudioDataThread(int size){
        if (hasReadAudioDataThread==null){
            mDrawHasReadAudioData=new AudioFileSurfaceView.DrawHasReadAudioData();
...
//						LogUtils.v("tempBuf[i]="+tempBuf[i]);
    ...
    public AudioFileSurfaceView(Context context, AttributeSet attrs) {
        super(context, attrs);
...
//		}
        ...
        canvas.drawText(mShowText, (int)((getWidth()-MARGIN_RIGHT/2)-textWidth/2), progressHeight , textPaint);
    }
